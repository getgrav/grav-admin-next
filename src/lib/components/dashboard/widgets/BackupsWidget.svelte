...
<div class="h-full rounded-lg border border-border bg-card p-4">
	<div class="mb-3 flex items-center justify-between gap-2">
		...
		{#if data().canWriteSystem}
			<button
				type="button"
				class="inline-flex h-6 items-center gap-1 rounded-md bg-primary px-2 text-[0.6875rem] font-semibold text-primary-foreground shadow-sm transition-colors hover:bg-primary/90 disabled:opacity-60"
			...
			{#each backups.slice(0, max) as backup}
				<div class="flex items-center gap-3 px-4 py-2">
					<div class="min-w-0 flex-1">
						<div class="text-[0.75rem] font-medium text-foreground">{formatDate(backup.date)}</div>
						<div class="text-[0.6875rem] text-muted-foreground">{formatBytes(backup.size)}</div>
					</div>
	...
	{:else}
		<p class="py-4 text-center text-[0.8125rem] text-muted-foreground">{i18n.t('ADMIN_NEXT.BACKUPS_WIDGET.NO_BACKUPS_FOUND')}</p>
	{/if}
</div>

<script lang="ts">
	...
	interface Props {
		/** 'sidebar' = small (28px), 'login' = large (56px) */
		size?: 'sidebar' | 'login';
		/** Whether to show the text label next to the logo */
		showLabel?: boolean;
	}
	...
	// Collapsed sidebar: the 52px rail has no room for the Grav wordmark, so crop
	// the viewBox to the circular badge (0..140) and pin the SVG to a square that
	// fits the rail. A custom logo can't be cropped, so it's constrained to fit
	// instead (contained in the square). (admin2#141)
	const iconOnly = $derived(isSmall && !showLabel);
	const logoHeight = $derived(isSmall ? 'h-7' : 'h-12');
	const logoViewBox = $derived(iconOnly ? '0 0 140 140' : '0 0 504 140');
	const logoClass = $derived(iconOnly ? 'h-7 w-7 shrink-0' : `${logoHeight} w-auto max-w-full`);
